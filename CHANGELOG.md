...
- Fixed `test_grants_idempotency` integration test macro: hardcoded `test_schema = 'PUBLIC'`, which caused CI failures with "Schema '...PUBLIC' does not exist or not authorized" once the test was wired into CI, since the CI Snowflake database has no accessible `PUBLIC` schema. Changed to use `target.schema`, which is always created by this project's own `dbt build` step.
- Fixed the `ref()` override signature: the `package` and `version` parameters added for cross-package/versioned ref support were inserted before `include_database`, silently breaking any existing caller using the previous positional form `ref('model_name', true)` (the `true` would bind to `package` instead of `include_database`). Reordered to `ref(model_name, include_database=false, package=none, version=none)`, preserving the original two-positional-argument compatibility while keeping `package`/`version` available as keyword arguments (per code review from Sourcery).
- Hardened `test_clean_macros` with two real, directly-asserted checks on the root-cause Snowflake casing behaviour (unquoted `SELECT ... AS alias` columns returned uppercase; `SHOW <OBJECT>` command columns returned lowercase) instead of relying solely on an unused `failures` list that could never actually fail (per code review from Sourcery). The remaining checks are explicitly documented as smoke tests (assert the macros don't raise, not their exact drop-selection output).
...
### Fixed
- Fixed `execute_task` macro: referenced undefined variable `root_task` instead of the `task_name` parameter, causing task execution to fail.

### Changed
- All grant, share, task, clean, and dependency macros now execute during `dbt build` in addition to `dbt run` and `dbt run-operation`. Previously `dbt build` was not recognised as a valid execution context, causing all on-run-end hooks to silently skip.
- Added informational logging to `grant_database_usage` macro.
