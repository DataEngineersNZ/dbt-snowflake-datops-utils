    ...
    {% if flags.WHICH in ['run', 'run-operation'] %}
        ...
        {% if execute %}
            ...
            {% if include_schemas | length > 0 %}
                {% do dbt_dataengineers_utils.grant_schema_operate_specific(include_schemas, grant_roles, true) %}
            {% endif %}
        {% endif %}
    {% endif %}
{% endmacro %}
       ...
       {% for schema in schemas %}
            ...
            {% for role in grant_roles %}
                {% if role not in existing_roles %}
                    ...
                    {{ queries.append(" grant operate on all pipes in schema " ~ target.database ~ "." ~ schema ~ " to role " ~ role ~ ";") }}
                    {{ queries.append(" grant operate on all tasks in schema " ~ target.database ~ "." ~ schema ~ " to role " ~ role ~ ";") }}
