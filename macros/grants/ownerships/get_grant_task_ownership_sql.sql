{% macro get_grant_task_ownership_sql(schema_list, role_name) %}
    ...
    {% if results and results | length > 0 %}
        {% for r in results %}
            {% do statements.append('grant ownership on task ' ~ target.database ~ '.' ~ r[0] ~ '.' ~ r[1] ~ ' to role ' ~ role_name ~ ' revoke current grants;') %}
        {% endfor %}
        {% do log('get_grant_task_ownership_sql: generated ' ~ (statements | length) ~ ' statements', info=True) %}
    {% else %}
        {% do log('get_grant_task_ownership_sql: no task ownership changes required', info=True) %}
    {% endif %}
