{% macro grant_schema_monitor_access(exclude_schemas, grant_roles) %}
    ...
    {% if flags.WHICH in ['run'] %}
        ...
        {% if execute %}
            ...
            {% if include_schemas | length > 0%}
                {% do grant_schema_monitor_access_specific(include_schemas, grant_roles, true) %}
            {% endif %}
        {% endif %}
    {% endif %}
{% endmacro %}

{% macro grant_schema_monitor_access_specific(schemas, grant_roles, revoke_current_grants) %}
    {% if flags.WHICH in ['run'] %}
       {% set existing_roles = []%}
       {% for schema in schemas %}
            {% do log("Granting and Revoking Schema Monitor Grants", info=True) %}
            {% set query %}
