{% macro unit_test(model, input_mapping, input_parameters, expected_output, test_name, description, compare_columns) %}
    {%if not input_parameters %}
        {% set test_sql = dbt_dataengineers_utils.get_model_unit_test_sql(model, input_mapping, none, test_name)|trim %}
    {% else %}
...
{% test unit_test(model, input_mapping, expected_output, test_name, description, compare_columns) %}
    {% do return(dbt_dataengineers_utils.unit_test(model, input_mapping, expected_output, test_name, description, compare_columns)) %}
{% endtest %}
